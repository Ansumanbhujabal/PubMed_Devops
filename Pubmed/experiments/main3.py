...
# def identify_academic_non_academic_authors(Affiliation:List[str],authors: List[str], emails: List[str]) -> Tuple[List[str], List[str]]:
#     academic_authors = []
#     non_academic_authors = []

#     for author, email in zip(authors, emails):
#         if not email:  # Handle missing emails
#             print(f"No email found for {author}. Classified as non-academic.")
#             non_academic_authors.append(author)
#         else:
#             email = email.strip()  # Remove any leading/trailing whitespace
#             # Check if the email domain indicates academic affiliation
#             if any(domain in email.lower() for domain in [".edu", ".ac", ".org", ".gov"]):
#                 print(f"Academic email found for {author}: {email}")
#                 academic_authors.append(author)
#             else:
#                 print(f"Non-academic email found for {author}: {email}")
#                 non_academic_authors.append(author)
...
def identify_academic_non_academic_authors(Affiliation: List[str], authors: List[str], emails: List[str]) -> Tuple[List[str], List[str]]:
    ...
    academic_keywords = ["university", "school", "institute", "college","department","Research"]

    for author, email, affiliation in zip(authors, emails, Affiliation):
        if not email:  # Handle missing emails
            print(f"No email found for {author}. Checking affiliation...")
            if any(keyword in affiliation.lower() for keyword in academic_keywords):
                print(f"Academic affiliation found for {author}: {affiliation}")
                academic_authors.append(author)
            else:
                print(f"No academic indicators for {author}. Classified as non-academic.")
                non_academic_authors.append(author)
        else:
            email = email.strip()  # Remove any leading/trailing whitespace
            # Check if the email domain indicates academic affiliation
            if any(domain in email.lower() for domain in [".edu", ".ac", ".org", ".gov"]) or \
               any(keyword in affiliation.lower() for keyword in academic_keywords):
                print(f"Academic indicator found for {author}: Email: {email}, Affiliation: {affiliation}")
                academic_authors.append(author)
            else:
                print(f"Non-academic email found for {author}: {email}")
                non_academic_authors.append(author)
    ...
    for chunk_i in range(0, len(id_list), chunk_size):
        ...
        for paper in papers:
            ...
            for author in paper["MedlineCitation"]["Article"]["AuthorList"]:
                ...
                # if "AffiliationInfo" in author and "Email" in author["AffiliationInfo"][0]:
                #     emails.append(author["AffiliationInfo"][0]["Email"])
                # else:
                #     emails.append("")  # Empty string if no email is provided
            ...
            # Identify academic and non-academic authors based on email and affiliation
            ...
            academic_authors, non_academic_authors = identify_academic_non_academic_authors( all_affiliations_list,authors,email_list)
